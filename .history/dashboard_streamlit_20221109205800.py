...
st.bar_chart(sat_v_abil, y='Ability', x='Satisfaction')

df_2 = pd.read_csv('data/5glhn8oo mergedSummaryExerciseData.csv')

st.subheader('Type of Exercise for Subject 2')
exercise = df_2[['HeavyEx', 'ModerateEx', 'LightEx']]
st.line_chart(exercise)

st.subheader('Code for Line Chart Comparison')
code = """st.subheader('Satisfaction vs. Ability for Subject 1')
    sat_v_abil = df_1[['Ability', 'Satisfaction']]
